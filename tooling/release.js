...
(async () => {
  await exec(
    `cd ./build; rm -f ${releaseZip(CHROME_CONSUMER)}; zip -rq ${releaseZip(CHROME_CONSUMER)} ./${CHROME_CONSUMER}/*`
  );
  await exec(
    `cd ./build; rm -f ${releaseZip(CHROME_ENTERPRISE)}; zip -rq ${releaseZip(
      CHROME_ENTERPRISE
    )} ./${CHROME_ENTERPRISE}/*`
  );
