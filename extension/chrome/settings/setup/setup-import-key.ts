...
export class SetupImportKeyModule {
    ...
    try {
      ...
      if (e instanceof UserAlert) {
        ...
        return await Ui.modal.error(
          `An error happened when processing the key: ${String(e)}\n${Lang.general.contactForSupportSentence(this.view.isCustomUrlFesUsed())}`,
          false,
  ...
  public renderCompatibilityFixBlockAndFinalizeSetup = async (origPrv: Key, options: SetupOptions) => {
    $('.ask_support_assistance_container').text(Lang.general.contactIfNeedAssistance(this.view.isCustomUrlFesUsed()));
    ...
    try {
      ...
      Catch.reportErr(e);
      await Ui.modal.error(`Failed to fix key (${String(e)}). ${Lang.general.writeMeToFixIt(this.view.isCustomUrlFesUsed())}`, false, Ui.testCompatibilityLink);
