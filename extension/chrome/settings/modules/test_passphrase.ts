...
Catch.try(async () => {
  ...
  $('.action_verify').click(Ui.event.handle(async () => {
    ...
    if (await Pgp.key.decrypt(key, [String($('#password').val())]) === true) {
      ...
      await Ui.modal.warning('Pass phrase didn\'t match. Please try again. If you forgot your pass phrase, please change it, so that do don\'t get locked out of your encrypted messages.');
    }
  }));
