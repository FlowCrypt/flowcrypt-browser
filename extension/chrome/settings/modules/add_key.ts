...
View.run(
  class AddKeyView extends View {
    ...
    private isCustomUrlFesUsed = () => Boolean(this.fesUrl);
      ...
      try {
        ...
        if (e instanceof UserAlert) {
          ...
          return await Ui.modal.error(
            `An error happened when processing the key: ${String(e)}\n${Lang.general.contactForSupportSentence(this.isCustomUrlFesUsed())}`,
            false,
    ...
    private renderCompatibilityFixBlockAndFinalizeSetup = async (origPrv: Key) => {
      ...
      try {
        ...
        Catch.reportErr(e);
        await Ui.modal.error(`Failed to fix key (${String(e)}). ${Lang.general.writeMeToFixIt(this.isCustomUrlFesUsed())}`, false, Ui.testCompatibilityLink);
