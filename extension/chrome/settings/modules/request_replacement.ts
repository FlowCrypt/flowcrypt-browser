...
Catch.try(async () => {
  ...
  if (!keyserverRes.pubkey || !keyserverRes.attested || Pgp.key.fingerprint(primaryPubkeyArmored) === Pgp.key.fingerprint(keyserverRes.pubkey)) {
    ...
    Xss.sanitizeRender('#status', `Original key KeyWords:<br/><span class="good">${mnemonic(Pgp.key.longid(keyserverRes.pubkey)!)}<br/>${Pgp.key.fingerprint(keyserverRes.pubkey, 'spaced')}</span>`); // all pubkeys on keyserver should have computable longid
