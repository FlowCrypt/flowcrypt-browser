...
View.run(class DebugApiView extends View {
  ...
  public render = async () => {
    if (this.which === 'google_account') {
      ...
      this.renderCallRes('Store.getAcct.openid', { acctEmail: this.acctEmail }, await AcctStore.getAcct(this.acctEmail, ['openid']));
      ...
      const storage = await AcctStore.getAcct(this.acctEmail, [
