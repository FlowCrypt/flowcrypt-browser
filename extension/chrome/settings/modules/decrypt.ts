...
View.run(
  class ManualDecryptView extends View {
    ...
    private readonly attachmentUi = new AttachmentUI(() =>
      ...
      Promise.resolve({ count: 1, size: 100 * 1024 * 1024, size_mb: 100 })
    );
    ...
    public setHandlers = () => {
      $('.action_decrypt_and_download').on(
        'click',
        this.setHandlerPrevent('double', el => this.actionDecryptAndDownloadHandler(el))
      );
    };
    ...
    private decryptAndDownload = async (encrypted: Attachment) => {
      // todo - this is more or less copy-pasted from attachment.js, should use common function
