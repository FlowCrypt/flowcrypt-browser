...
export class InboxActiveThreadModule extends ViewModule<InboxView> {
  ...
  private renderMsg = async (message: GmailRes.GmailMsg) => {
    ...
    try {
      ...
      for (const block of blocks) {
        if (
          block.type === 'encryptedMsg' ||
          block.type === 'publicKey' ||
          block.type === 'privateKey' ||
      ...
      const exportBtn = this.debugEmails.includes(this.view.acctEmail)
        ? '<a href="#" class="action-export">download api export</a>'
        : '';
      r =
        `<p class="message_header" data-test="container-msg-header">From: ${Xss.escape(
          from
        )} <span style="float:right;">${headers.date} ${exportBtn}</p>` + r;
      ...
      if (ApiErr.isNetErr(e)) {
        Xss.sanitizeAppend(
          '.thread',
          this.wrapMsg(htmlId, `Failed to load a message (network error), skipping. ${Ui.retryLink()}`)
        );
      } else if (ApiErr.isAuthErr(e)) {
        this.view.inboxNotificationModule.renderAndHandleAuthPopupNotification();
      } else if (ApiErr.isMailOrAcctDisabledOrPolicy(e)) {
        ...
        Xss.sanitizeAppend(
          '.thread',
          this.wrapMsg(htmlId, `Failed to load a message due to the following error: <pre>${printable}</pre>`)
        );
      }
    }
  };
  ...
  private exportMsgForDebug = async (msgId: string) => {
    ...
    Browser.saveToDownloads(
      new Attachment({ data: Buf.fromUtfStr(json), type: 'application/json', name: `message-export-${msgId}.json` })
    );
  };
  ...
  private renderReplyBox = (replyMsgId: string) => {
    ...
    this.view.S.cached('thread').append(
      Ui.e('div', { class: 'reply line', html: this.view.factory.embeddedReply(params, false, false) })
    ); // xss-safe-factory
  };
