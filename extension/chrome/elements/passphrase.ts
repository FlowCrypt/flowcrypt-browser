...
View.run(class PassphraseView extends View {
  ...
  public setHandlers = () => {
    ...
    $('.action_passphrase_hint').hover(this.setHandler((el) => this.showPassphraseHintDialog(el)));
  }
  ...
  private showPassphraseHintDialog = async (el: HTMLElement) => {
    ...
    if (allPrivateKeys.length > 1) {
      if (this.keysWeNeedPassPhraseFor.length === 1) {
        ...
        passphraseDialogMessage = `Pass phrase for a key with fingerprint ${Str.spaced(fingerprint)} and\nwith the type of ${keyType} is required to proceed.`;
      } else {
