...
import { Store, ContactUpdate, DbContactFilter, AccountStoreExtension, SendAsAlias, DbContactObjArg } from '../../js/common/platform/store.js';
...
Catch.try(async () => {
  ...
  const composer = new Composer({
    ...
    storageGetAddressesKeyserver() {
      return storage.addresses_keyserver || [];
    },
    storageGetAddresses,
    storageEmailFooterGet() {
      return storage.email_footer || undefined;
    },
    async storageEmailFooterSet(footer: string | undefined) {
      storage.email_footer = footer;
      await Store.setAcct(acctEmail, { email_footer: footer });
    },
    storageGetHideMsgPassword() {
      return !!storage.hide_message_password;
    },
    storageGetSubscription() {
      return Store.subscription();
    },
    async storageGetKey(senderEmail: string): Promise<KeyInfo> {
      const [primaryKi] = await Store.keysGet(acctEmail, ['primary']);
      Assert.abortAndRenderErrorIfKeyinfoEmpty(primaryKi);
      return primaryKi;
    },
    async storageSetDraftMeta(storeIfTrue: boolean, draftId: string, threadId: string, recipients: string[], subject: string) {
    ...
    async storageAddAdminCodes(shortId: string, msgAdminCode: string, attAdminCodes: string[]) {
    ...
    storageContactGet(email: string[]) {
      return Store.dbContactGet(undefined, email);
    },
    storageContactUpdate(email: string[] | string, update: ContactUpdate) {
      return Store.dbContactUpdate(undefined, email, update);
    },
    storageContactSave(contact: Contact) {
      return Store.dbContactSave(undefined, contact);
    },
    storageContactSearch(query: DbContactFilter) {
      return Store.dbContactSearch(undefined, query);
    },
    storageContactObj(object: DbContactObjArg) {
      return Store.dbContactObj(object);
    },
    emailProviderDraftGet(draftId: string) {
      return Google.gmail.draftGet(acctEmail, draftId, 'raw');
    },
    emailProviderDraftCreate(acctEmail: string, mimeMsg: string, threadID: string) {
      return Google.gmail.draftCreate(acctEmail, mimeMsg, threadID);
    },
    emailProviderDraftUpdate(draftId: string, mimeMsg: string) {
      return Google.gmail.draftUpdate(acctEmail, draftId, mimeMsg);
    },
    emailProviderDraftDelete(draftId: string) {
      return Google.gmail.draftDelete(acctEmail, draftId);
    },
    emailProviderMsgSend(message: SendableMsg, renderUploadProgress: ProgressCb) {
      return Google.gmail.msgSend(acctEmail, message, renderUploadProgress);
    },
    ...
    async emailProviderDetermineReplyMsgHeaderVariables(progressCb?: ProgressCb): Promise<DeterminedMsgHeaders | undefined> {
      try {
    ...
    emailProviderExtractArmoredBlock(msgId: string) {
      return Google.gmail.extractArmoredBlock(acctEmail, msgId, 'full');
    },
    ...
    renderFooterDialog() {
      return ($ as JQS).featherlight({ // tslint:disable:no-unsafe-any
    ...
    renderHelpDialog() {
      BrowserMsg.send.bg.settings({ acctEmail, page: '/chrome/settings/modules/help.htm' });
    },
    closeMsg() {
    ...
    factoryAtt(att: Att, isEncrypted: boolean) {
      return factory.embeddedAtta(att, isEncrypted);
    },
    ...
    async lookupPubkeyFromDbOrKeyserverAndUpdateDbIfneeded(email: string): Promise<Contact | "fail"> {
      const [dbContact] = await this.storageContactGet([email]);
    ...
    async collectAllAvailablePublicKeys(acctEmail: string, recipients: string[]):
      Promise<{ armoredPubkeys: PubkeyResult[], emailsWithoutPubkeys: string[] }> {
      const contacts = await this.storageContactGet(recipients);
      const { public: senderArmoredPubkey } = await this.storageGetKey(acctEmail);
