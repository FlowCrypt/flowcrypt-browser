...
import { FormatError, PgpUtil, DecryptErrTypes } from '../../../js/common/core/crypto/pgp/pgp-msg.js';
...
export class ComposeQuoteModule extends ViewModule<ComposeView> {
        ...
        if (block.type === 'decryptedHtml') {
          ...
          if (block.attMeta?.data) {
            ...
            if (block.type === 'encryptedAtt') {
              this.setQuoteLoaderProgress('decrypting...');
              const result = await PgpUtil.decryptMessage({ kisWithPp: await KeyStore.getAllWithPp(this.view.acctEmail), encryptedData: block.attMeta.data });
              if (result.success) {
  ...
  private decryptMessage = async (encryptedData: Buf): Promise<string> => {
    const decryptRes = await PgpUtil.decryptMessage({ kisWithPp: await KeyStore.getAllWithPp(this.view.acctEmail), encryptedData });
