...
import { PgpMsgUtil } from '../../../../js/common/core/crypto/pgp/pgp-msg-util.js';
...
export class SignedMsgMailFormatter extends BaseMailFormatter {
  ...
  public sendableMsg = async (newMsg: NewMsgData, signingPrv: Key): Promise<SendableMsg> => {
    ...
    if (!this.richtext) {
      ...
      const signedData = await PgpMsgUtil.sign(signingPrv, newMsg.plaintext);
    ...
    sendable.setSignMethod((signable: string) => PgpMsgUtil.sign(signingPrv, signable, true));
    return sendable;
  }
