...
export class EncryptedMsgMailFormatter extends BaseMailFormatter {
  ...
  private sendablePwdMsg = async (newMsg: NewMsgData, pubs: PubkeyResult[], short: string, signingPrv?: Key) => {
    ...
    //return await SendableMsg.createOpenPGP(this.acctEmail, { ...this.headers(newMsg), body: emailIntroAndLinkBody, atts, isDraft: this.isDraft });
  ...
  private sendableSimpleTextMsg = async (newMsg: NewMsgData, pubs: PubkeyResult[], signingPrv?: Key): Promise<SendableMsg> => {
    ...
    if (x509certs.length) { // s/mime
      ...
      return await SendableMsg.createSMime(this.acctEmail, this.headers(newMsg), body, { isDraft: this.isDraft });
    }
    // openpgp
    ...
    //return await SendableMsg.createSMimeOrOpenPGP(this.acctEmail, { ...this.headers(newMsg), body: { "encrypted/buf": Buf.fromUint8(encryptedBody) }, type: mimeType, atts, isDraft: this.isDraft });
    return await SendableMsg.createPgpInline(this.acctEmail, this.headers(newMsg), { "encrypted/buf": Buf.fromUint8(encryptedBody) }, atts, { type: mimeType, isDraft: this.isDraft });
  }
