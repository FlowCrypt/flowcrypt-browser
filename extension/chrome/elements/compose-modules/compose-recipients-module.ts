...
export class ComposeRecipientsModule extends ViewModule<ComposeView> {
  ...
  public evaluateRecipients = async (recipientEls: ValidRecipientElement[], triggerCallback = true) => {
    ...
    await Promise.all(recipientEls.map(r => r.evaluating).filter((p): p is Promise<void> => p !== undefined));
    if (triggerCallback) {
  ...
  public collapseInputsIfNeeded = async () => {
    ...
    await Promise.all(this.addedRecipients.map(r => r.evaluating).filter((p): p is Promise<void> => p !== undefined)); // Wait until all recipients loaded.
