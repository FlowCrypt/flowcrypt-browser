    ...
    if (pubs.length > 1) {
      ...
      for (const pubkey of pubs) {
        ...
        if (Str.isEmailValid(email)) {
          contacts.push(await Store.dbContactObj({ email, client: 'pgp', pubkey: pubkey.armor(), lastUse: Date.now() }));
        }
      }
      ...
      if (Str.isEmailValid(String($('.input_email').val()))) {
        const contact = await Store.dbContactObj({ email: String($('.input_email').val()), client: 'pgp', pubkey: pubs[0].armor(), lastUse: Date.now() });
