  ...
  private renderPgpSignatureCheckMissingPubkeyOptions = async (signerLongid: string, senderEmail: string | undefined): Promise<void> => {
    ...
    try {
      if (senderEmail) { // we know who sent it
        const [senderContactByEmail] = await ContactStore.dbContactGet(undefined, [senderEmail]);
        ...
        await ContactStore.dbContactSave(undefined, await ContactStore.dbContactObj({ email: senderEmail, pubkey, client: pgpClient })); // <= TOFU auto-import
        ...
        const [conflictingContact] = await ContactStore.dbContactGet(undefined, [pubkeyEmail]);
        ...
        render(`Fetched matching pubkey ${signerLongid}. Click to load and use it.`, async () => {
          await ContactStore.dbContactSave(undefined, await ContactStore.dbContactObj({ email: pubkeyEmail, pubkey, client: pgpClient })); // TOFU manual import
