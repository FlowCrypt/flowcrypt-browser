...
export class PgpBlockViewAttachmentsModule {
  ...
  private decryptAndSaveAttToDownloads = async (encrypted: Att, renderIn: JQuery<HTMLElement>) => {
    ...
    const decrypted = await BrowserMsg.send.await.pgpMsgDecrypt(this.view.parentTabId, { kisWithPp, encryptedData: encrypted.getData(), msgPwd: await this.view.pwdEncryptedMsgModule.getDecryptPwd() });
