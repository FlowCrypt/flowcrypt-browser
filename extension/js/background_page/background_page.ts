...
(async () => {
  ...
  let open_settings_page_handler: BrowserMsgHandler = async (message: {path: string, account_email: string, page: string, page_url_params: Dict<FlatTypes>, add_new_account?: boolean}, sender, respond) => {
  ...
  let get_active_tab_info: BrowserMsgHandler = (message: Dict<any>|null, sender, respond) => {
  ...
  let db_operation = (request: BrowserMsgReqtDb, sender: chrome.runtime.MessageSender|'background', respond: Function, db: IDBDatabase) => { // tslint:disable-line:ban-types
    Catch.try(() => {
      if (db) {
  ...
  BrowserMsg.listen_background({
    ...
    db: (request, sender, respond) => db_operation(request as BrowserMsgReqtDb, sender, respond, db),
    session_set: (r: BrowserMsgReqSessionSet, sender, respond) => Store.session_set(r.account_email, r.key, r.value).then(respond).catch(Catch.rejection),
    session_get: (r: BrowserMsgReqSessionGet, sender, respond) => Store.session_get(r.account_email, r.key).then(respond).catch(Catch.rejection),
