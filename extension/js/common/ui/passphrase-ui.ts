...
export const initPassphraseToggle = async (passphraseInputIds: string[], forceInitialShowOrHide?: 'show' | 'hide') => {
  ...
  for (const id of passphraseInputIds) {
    ...
    if (show) {
      passphraseInput.after(
        `<label href="#" id="toggle_${id}" class="toggle_show_hide_pass_phrase" for="${id}">${buttonHide}</label>`
      );
      passphraseInput.attr('type', 'text');
    } else {
      passphraseInput.after(
        `<label href="#" id="toggle_${id}" class="toggle_show_hide_pass_phrase" for="${id}">${buttonShow}</label>`
      );
