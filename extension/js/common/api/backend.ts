...
export class Backend extends Api {
  ...
  private static call = (path: string, vals: Dict<any>, fmt: ReqFmt = 'JSON'): Promise<any> => {
    return Backend.apiCall(Backend.fc.url('api'), path, vals, fmt, undefined, { 'api-version': '3' });
  }
  ...
  public static fc = {
    ...
    helpFeedback: (acctEmail: string, message: string): Promise<BackendRes.FcHelpFeedback> => Backend.call('help/feedback', {
      email: acctEmail,
      message,
    }),
    helpUninstall: (email: string, client: string): Promise<unknown> => Backend.call('help/uninstall', {
      email,
      client,
    ...
    accountCheck: (emails: string[]) => Backend.call('account/check', {
      emails,
    ...
    accountCheckSync: async () => { // callbacks true on updated, false not updated, null for could not fetch
      ...
      if (emails.length) {
        const response = await Backend.fc.accountCheck(emails);
        const authInfo = await Store.authInfo();
    ...
    accountUpdate: async (updateValues?: Dict<Serializable>): Promise<BackendRes.FcAccountUpdate> => {
      const authInfo = await Store.authInfo();
    ...
    accountSubscribe: async (product: string, method: string, paymentSourceToken?: string): Promise<BackendRes.FcAccountSubscribe> => {
      const authInfo = await Store.authInfo();
    ...
    messagePresignFiles: async (atts: Att[], authMethod?: FcAuthMethods): Promise<BackendRes.FcMsgPresignFiles> => {
    ...
    messageConfirmFiles: (identifiers: string[]): Promise<BackendRes.FcMsgConfirmFiles> => Backend.call('message/confirm_files', {
      identifiers,
    }),
    messageUpload: async (encryptedDataArmored: string, authMethod?: FcAuthMethods): Promise<BackendRes.FcMsgUpload> => { // todo - DEPRECATE THIS. Send as JSON to message/store
    ...
    messageToken: async (): Promise<BackendRes.FcMsgToken> => {
      const authInfo = await Store.authInfo();
      return await Backend.call('message/token', { account: authInfo.acctEmail, uuid: authInfo.uuid }) as BackendRes.FcMsgToken;
    },
    messageExpiration: async (adminCodes: string[], addDays?: number): Promise<BackendRes.ApirFcMsgExpiration> => {
      const authInfo = await Store.authInfo();
    ...
    messageReply: (short: string, token: string, from: string, to: string, subject: string, message: string): Promise<unknown> => Backend.call('message/reply', {
      short,
      token,
      from,
      to,
    ...
    messageContact: (sender: string, message: string, messageToken: FcAuthToken): Promise<unknown> => Backend.call('message/contact', {
      message_token_account: messageToken.account,
      message_token: messageToken.token,
      sender,
      message,
    }),
    linkMessage: (short: string): Promise<BackendRes.FcLinkMsg> => Backend.call('link/message', {
      short,
    }),
    linkMe: (alias: string): Promise<BackendRes.FcLinkMe> => Backend.call('link/me', {
      alias,
    }),
  };
