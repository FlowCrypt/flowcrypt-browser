...
export class KeyStoreUtil {
  ...
  public static chooseMostUseful = (
    prvs: ParsedKeyInfo[],
    criteria: 'ONLY-FULLY-USABLE' | 'AT-LEAST-USABLE-BUT-EXPIRED' | 'EVEN-IF-UNUSABLE'
  ): ParsedKeyInfo | undefined => {
    const usablePrv =
      prvs.find(prv => prv.key.usableForEncryption && prv.key.usableForSigning) ||
    ...
    return prvs.find(prv => !prv.key.revoked) || prvs[0];
  };
}
