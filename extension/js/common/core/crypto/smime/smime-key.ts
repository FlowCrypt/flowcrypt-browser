...
export class SmimeKey {
  ...
  public static parseDecryptBinary(buffer: Uint8Array, password: string): Key {
    ...
    const asn1 = SmimeKey.forgeCustom.asn1.fromDer(bytes);
    let certificate: forge.pki.Certificate | undefined;
    try {
      // try to recognize a certificate
      certificate = SmimeKey.forgeCustom.pki.certificateFromAsn1(asn1);
    } catch (e) {
      // fall back to p12
    }
    if (certificate) {
    ...
    const p12 = SmimeKey.forgeCustom.pkcs12.pkcs12FromAsn1(asn1, password);
    const certBags = p12.getBags({ bagType: SmimeKey.forgeCustom.pki.oids.certBag });
    if (!certBags) {
      throw new Error('No user certificate found.');
    }
    const certBag = certBags[SmimeKey.forgeCustom.pki.oids.certBag];
    if (!certBag) {
      throw new Error('No user certificate found.');
    }
    ...
    const keyBags = (
      p12.getBags({ bagType: SmimeKey.forgeCustom.pki.oids.pkcs8ShroudedKeyBag })[SmimeKey.forgeCustom.pki.oids.pkcs8ShroudedKeyBag] ?? []
    ).concat(p12.getBags({ bagType: SmimeKey.forgeCustom.pki.oids.keyBag })[SmimeKey.forgeCustom.pki.oids.keyBag] ?? []);
  ...
  public static async encryptMessage({
    ...
    const p7 = SmimeKey.forgeCustom.pkcs7.createEnvelopedData();
    ...
    p7.content = SmimeKey.forgeCustom.util.createBuffer(input);
    p7.encrypt();
    let data: Uint8Array;
    if (armor) {
      data = Buf.fromRawBytesStr(SmimeKey.forgeCustom.pkcs7.messageToPem(p7));
    } else {
  ...
  public static readArmoredPkcs7Message(
    ...
    return SmimeKey.forgeCustom.pkcs7.messageFromPem(Str.with(encrypted));
  }
  ...
  public static decryptMessage(p7: forge.pkcs7.PkcsEnvelopedData, key: Key): Uint8Array {
    ...
    const decryptedPrivateKey = SmimeKey.forgeCustom.pki.privateKeyFromPem(armoredPrivateKey);
  ...
  public static async sign(signingPrivate: Key, data: Uint8Array): Promise<Uint8Array> {
    ...
    const p7 = SmimeKey.forgeCustom.pkcs7.createSignedData();
    p7.addSigner({
    ...
    p7.content = SmimeKey.forgeCustom.util.createBuffer(data);
    p7.sign();
  ...
  public static async decryptKey(key: Key, passphrase: string, optionalBehaviorFlag?: 'OK-IF-ALREADY-DECRYPTED'): Promise<boolean> {
    ...
    const privateKey = await SmimeKey.forgeCustom.pki.decryptRsaPrivateKey(encryptedPrivateKey, passphrase); // null on password mismatch
    if (!privateKey) {
      return false;
    }
  ...
  public static async encryptKey(key: Key, passphrase: string) {
    ...
    const encryptedPrivateKey = SmimeKey.forgeCustom.pki.encryptRsaPrivateKey(SmimeKey.forgeCustom.pki.privateKeyFromPem(armoredPrivateKey), passphrase);
  ...
  public static getMessageLongids(msg: SmimeMsg): string[] {
    return msg.recipients.map(recipient => {
      ...
      const der = SmimeKey.forgeCustom.asn1.toDer(asn1).getBytes();
  ...
  private static messageToDer(p7: forge.pkcs7.Pkcs7Data): Uint8Array {
    ...
    return Buf.fromRawBytesStr(SmimeKey.forgeCustom.asn1.toDer(asn1).getBytes());
  }
  ...
  private static getLeafCertificates(msgBlocks: MsgBlock[]): { pem: string; certificate: forge.pki.Certificate }[] {
    const parsed = msgBlocks.map(cert => {
      return { pem: cert.content as string, certificate: SmimeKey.forgeCustom.pki.certificateFromPem(cert.content as string) };
    });
  ...
  private static getKeyFromCertificate(certificateOrText: forge.pki.Certificate | string, privateKey: forge.pki.PrivateKey | string | undefined): Key {
    const certificate = typeof certificateOrText === 'string' ? SmimeKey.forgeCustom.pki.certificateFromPem(certificateOrText) : certificateOrText;
    ...
    if (typeof privateKey === 'string') {
      if (privateKey.includes(PgpArmor.headers('pkcs8EncryptedPrivateKey').begin)) {
        ...
        const unencryptedKey = SmimeKey.forgeCustom.pki.privateKeyFromPem(privateKey);
        if (!unencryptedKey) {
    ...
    const fingerprint = SmimeKey.forgeCustom.pki.getPublicKeyFingerprint(certificate.publicKey, { encoding: 'hex' }).toUpperCase();
    ...
    const issuerAndSerialNumberAsn1 = SmimeKey.createIssuerAndSerialNumberAsn1(
      SmimeKey.forgeCustom.pki.distinguishedNameToAsn1(certificate.issuer),
    ...
    const key = {
      ...
      revoked: false,
      issuerAndSerialNumber: SmimeKey.forgeCustom.asn1.toDer(issuerAndSerialNumberAsn1).getBytes(),
    } as Key;
  ...
  private static attributesToDistinguishedNameAsn1(attributes: forge.pki.Attribute[]): forge.asn1.Asn1 {
    return SmimeKey.forgeCustom.asn1.create(
      SmimeKey.forgeCustom.asn1.Class.UNIVERSAL,
      SmimeKey.forgeCustom.asn1.Type.SEQUENCE,
      true,
      attributes.map(attr => {
        const valueTagClass = attr.valueTagClass || SmimeKey.forgeCustom.asn1.Type.PRINTABLESTRING;
        return SmimeKey.forgeCustom.asn1.create(SmimeKey.forgeCustom.asn1.Class.UNIVERSAL, SmimeKey.forgeCustom.asn1.Type.SET, true, [
          SmimeKey.forgeCustom.asn1.create(SmimeKey.forgeCustom.asn1.Class.UNIVERSAL, SmimeKey.forgeCustom.asn1.Type.SEQUENCE, true, [
            // AttributeType
            SmimeKey.forgeCustom.asn1.create(
              SmimeKey.forgeCustom.asn1.Class.UNIVERSAL,
              SmimeKey.forgeCustom.asn1.Type.OID,
              false,
              SmimeKey.forgeCustom.asn1.oidToDer(attr.type).getBytes()
            ),
            // AttributeValue
            SmimeKey.forgeCustom.asn1.create(
              SmimeKey.forgeCustom.asn1.Class.UNIVERSAL,
              attr.valueTagClass,
              false,
              valueTagClass === SmimeKey.forgeCustom.asn1.Type.UTF8 ? SmimeKey.forgeCustom.util.encodeUtf8(attr.value) : attr.value
            ),
          ]),
        ]);
      })
  ...
  private static createIssuerAndSerialNumberAsn1(issuerAsn1: forge.asn1.Asn1, serialNumberHex: string) {
    return SmimeKey.forgeCustom.asn1.create(SmimeKey.forgeCustom.asn1.Class.UNIVERSAL, SmimeKey.forgeCustom.asn1.Type.SEQUENCE, true, [
      // Issuer
      issuerAsn1,
      // Serial
      SmimeKey.forgeCustom.asn1.create(
        SmimeKey.forgeCustom.asn1.Class.UNIVERSAL,
        SmimeKey.forgeCustom.asn1.Type.INTEGER,
        false,
        SmimeKey.forgeCustom.util.hexToBytes(serialNumberHex)
      ),
    ]);
  }
  ...
  private static getCertificate(key: Key) {
    return SmimeKey.forgeCustom.pki.certificateFromPem(SmimeKey.getArmoredCertificate(key));
  }
  ...
  private static saveArmored(key: Key, certificate: forge.pki.Certificate | string, privateKey: forge.pki.PrivateKey | string | undefined) {
    ...
    if (privateKey) {
      let armoredPrivateKey = typeof privateKey === 'string' ? privateKey : SmimeKey.forgeCustom.pki.privateKeyToPem(privateKey);
    ...
    let armoredCertificate = typeof certificate === 'string' ? certificate : SmimeKey.forgeCustom.pki.certificateToPem(certificate);
