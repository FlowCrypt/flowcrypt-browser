...
export class MessageRenderer {
  ...
  private readonly processedMessages = new Map<string, Promise<ProcessedMessage>>();
  private readonly resolvedMessages = new Map<string, { value: ProcessedMessage; expiration: number }>();
  ...
  public deleteExpired = (): void => {
    for (const [key, entry] of this.resolvedMessages) {
      if (entry.expiration <= Date.now()) this.resolvedMessages.delete(key);
    }
    this.downloader.deleteExpired();
  };

  public msgGetProcessed = async (msgId: string): Promise<ProcessedMessage> => {
    const cached = this.resolvedMessages.get(msgId);
    if (cached && cached.expiration > Date.now()) return cached.value;
    const existing = this.processedMessages.get(msgId);
    if (existing) return await existing;
    // Processed messages contain class instances and must not cross extension storage.
    const pending = (async () => {
      const value = await this.processFull(await this.downloader.msgGetFull(msgId));
      this.resolvedMessages.set(msgId, { value, expiration: Date.now() + 24 * 60 * 60 * 1000 });
      return value;
    })().finally(() => {
      this.processedMessages.delete(msgId);
    });
    this.processedMessages.set(msgId, pending);
