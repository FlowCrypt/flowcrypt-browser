...
export class BgExec {
  ...
  private static arg_object_urls_consume = (args: any[]) => args.map((arg: any) => BgExec.is_object_url(arg) ? Att.methods.objUrlConsume(arg) : arg);

  private static arg_object_urls_create = (args: any[]) => args.map(arg => BgExec.should_be_object_url(arg) ? Att.methods.objUrlCreate(arg) : arg);
