...
export class ComposerSendBtn extends ComposerComponent {
    ...
    private extractProcessSendMsg = async () => {
        ...
        try {
            ...
            const { armoredPubkeys, emailsWithoutPubkeys } = await this.app.collectAllAvailablePublicKeys(this.composer.getSender(), recipientsEmails);
            let mailFormatter: MailFormatter | undefined;
            if (this.encryptionType === 'signed') {
                this.composer.S.now('send_btn_text').text('Signing');
                const signingPrv = await this.getDecryptedPrimaryPrvOrShowError(this.composer.getSender());
                if (signingPrv) {
                ...
                let senderKey: KeyInfo | undefined;
                let signingPrv: OpenPGP.key.Key | undefined;
                if (this.encryptionType === 'encryptedAndSigned') {
                    senderKey = await this.app.storageGetKey(this.urlParams.acctEmail, this.composer.getSender());
                    signingPrv = await this.getDecryptedPrimaryPrvOrShowError(senderKey);
                }
                if (emailsWithoutPubkeys.length) {
                    await this.throwIfEncryptionPasswordInvalid(senderKey, newMsgData);
                }
    ...
    private getDecryptedPrimaryPrvOrShowError = async (senderEmailOrKey: string | KeyInfo): Promise<OpenPGP.key.Key | undefined> => {
        if (typeof senderEmailOrKey === 'string') {
            senderEmailOrKey = await this.app.storageGetKey(this.urlParams.acctEmail, senderEmailOrKey);
        }
        if (senderEmailOrKey) {
            const { keys: [prv] } = await openpgp.key.readArmored(senderEmailOrKey.private);
            const passphrase = await this.app.storagePassphraseGet(senderEmailOrKey);
            if (typeof passphrase === 'undefined' && !prv.isFullyDecrypted()) {
                ...
                if ((typeof await this.app.whenMasterPassphraseEntered(60)) !== 'undefined') { // pass phrase entered
                    return await this.getDecryptedPrimaryPrvOrShowError(senderEmailOrKey);
    ...
    private throwIfEncryptionPasswordInvalid = async (senderKey: KeyInfo | undefined, { subject, pwd }: { subject: string, pwd?: Pwd} ) => {
        if (pwd && pwd.answer) {
            const pp = await this.app.storagePassphraseGet(senderKey);
