          ...
          if (msgId) {
            if (this.canReadEmails) {
              ...
              $(newPgpAtts).prepend(this.factory.embeddedAttaStatus(statusMsg)).children('a.auth_settings').click(Ui.event.handle(async () => { // xss-safe-factory
                await Browser.openSettingsPageWithChromeTabs(undefined, this.acctEmail, '/chrome/settings/modules/auth_denied.htm');
              }));
            }
          } else {
  ...
  private addSettingsBtn = () => {
    if (window.location.hash.startsWith('#settings')) {
      ...
      if (settingsBtnContainer.length && !settingsBtnContainer.find('#fc_settings_btn').length) {
        ...
        settingsBtnContainer.find('#fc_settings_btn').click(Ui.event.handle(async () => await Browser.openSettingsPageWithChromeTabs(undefined, this.acctEmail)));
      }
    }
  }
