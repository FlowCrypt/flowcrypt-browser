...
export const defineSetupTests = (testVariant: TestVariant, testWithBrowser: TestWithBrowser, testWithSemaphoredGlobalBrowser: TestWithGlobalBrowser) => {
  ...
  if (testVariant !== 'CONSUMER-LIVE-GMAIL') {
    ...
    ava.test('[standalone] settings > login > close oauth window > close popup', testWithBrowser(async (t, browser) => {
    ...
    ava.test('[standalone] setup - import key - do not submit - did not use before', testWithBrowser(async (t, browser) => {
    ...
    ava.test('[standalone] setup - import key - submit - used before', testWithBrowser(async (t, browser) => {
    ...
    ava.test('[standalone] setup - import key - naked - choose my own pass phrase', testWithBrowser(async (t, browser) => {
    ...
    ava.test('[standalone] setup - import key - naked - auto-generate a pass phrase', testWithBrowser(async (t, browser) => {
      const settingsPage = await BrowserRecipe.openSettingsLoginApprove(t, browser, 'flowcrypt.test.key.import.naked@gmail.com');
    ...
    ava.test('[standalone] setup - import key - fix key self signatures', testWithBrowser(async (t, browser) => {
      const settingsPage = await BrowserRecipe.openSettingsLoginApprove(t, browser, 'flowcrypt.test.key.imported@gmail.com');
      await SetupPageRecipe.manualEnter(settingsPage, 'missing.self.signatures', { submitPubkey: false, fixKey: true });
    }));

    ava.test('[standalone] setup - import key - fix key self signatures - skip invalid uid', testWithBrowser(async (t, browser) => {
      const settingsPage = await BrowserRecipe.openSettingsLoginApprove(t, browser, 'flowcrypt.test.key.imported@gmail.com');
    ...
    ava.test.skip('[standalone] setup - create key advanced - no backup', testWithBrowser(async (t, browser) => {
    ...
    ava.test('[standalone] setup - recover with a pass phrase - skip remaining', testWithBrowser(async (t, browser) => {
    ...
    ava.test('[standalone] setup - recover with a pass phrase - 1pp1 then 2pp1', testWithBrowser(async (t, browser) => {
      const settingsPage = await BrowserRecipe.openSettingsLoginApprove(t, browser, 'flowcrypt.compatibility@gmail.com');
    ...
    ava.test('[standalone] setup - recover with a pass phrase - 2pp1 then 1pp1', testWithBrowser(async (t, browser) => {
      const settingsPage = await BrowserRecipe.openSettingsLoginApprove(t, browser, 'flowcrypt.compatibility@gmail.com');
    ...
    ava.test('[standalone] setup - recover with a pass phrase - 2pp1 then 1pp2', testWithBrowser(async (t, browser) => {
      const settingsPage = await BrowserRecipe.openSettingsLoginApprove(t, browser, 'flowcrypt.compatibility@gmail.com');
      await SetupPageRecipe.recover(settingsPage, 'flowcrypt.compatibility.2pp1', { hasRecoverMore: true, clickRecoverMore: true });
      await SetupPageRecipe.recover(settingsPage, 'flowcrypt.compatibility.1pp2');
    }));

    ava.test('[standalone] setup - recover with a pass phrase - 1pp1 then 1pp2 (shows already recovered), then 2pp1', testWithBrowser(async (t, browser) => {
      const settingsPage = await BrowserRecipe.openSettingsLoginApprove(t, browser, 'flowcrypt.compatibility@gmail.com');
      await SetupPageRecipe.recover(settingsPage, 'flowcrypt.compatibility.1pp1', { hasRecoverMore: true, clickRecoverMore: true });
    ...
    ava.test('[standalone] setup - recover with a pass phrase - no remaining', testWithBrowser(async (t, browser) => {
    ...
    ava.test('[standalone] setup - fail to recover with a wrong pass phrase', testWithBrowser(async (t, browser) => {
      const settingsPage = await BrowserRecipe.openSettingsLoginApprove(t, browser, 'flowcrypt.test.key.recovered@gmail.com');
      await SetupPageRecipe.recover(settingsPage, 'flowcrypt.wrong.passphrase', { hasRecoverMore: false, wrongPp: true });
    }));

    ava.test('[standalone] setup - fail to recover with a wrong pass phrase at first, then recover with good pass phrase', testWithBrowser(async (t, browser) => {
      const settingsPage = await BrowserRecipe.openSettingsLoginApprove(t, browser, 'flowcrypt.test.key.recovered@gmail.com');
    ...
    ava.test('[standalone] setup - import key - submit - offline - retry', testWithBrowser(async (t, browser) => {
      const settingsPage = await BrowserRecipe.openSettingsLoginApprove(t, browser, 'flowcrypt.test.key.used.pgp@gmail.com');
