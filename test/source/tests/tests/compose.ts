...
export const defineComposeTests = (testVariant: TestVariant, testWithNewBrowser: TestWithNewBrowser, testWithSemaphoredGlobalBrowser: TestWithGlobalBrowser) => {
  ...
  if (testVariant !== 'CONSUMER-LIVE-GMAIL') {
    ...
    ava.default.only('compose[global:flowcrypt.test.key.new.manual@gmail.com] - standalone - own key expired', testWithNewBrowser(async (t, browser) => {
      ...
      let settingsPage = await BrowserRecipe.openSettingsLoginApprove(t, browser, 'flowcrypt.test.key.new.manual@gmail.com');
      ...
      await settingsPage.close();
      const composePage = await ComposePageRecipe.openStandalone(t, browser, 'flowcrypt.test.key.new.manual@gmail.com');
      ...
      await Util.sleep(0.5);
      const errorModal = await composePage.target.$('.ui-modal-error');
      const settingsUpdateURL = await getElementPropertyJson((await errorModal!.$('#action_update_prv'))!, 'href');
      await Util.sleep(0.5);
      settingsPage = await browser.newPage(t, settingsUpdateURL);
      await Util.sleep(0.5);
      const updatePrvPage = await settingsPage.getFrame(['my_key_update.htm']);
