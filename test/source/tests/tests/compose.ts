...
export const defineComposeTests = (testVariant: TestVariant, testWithBrowser: TestWithBrowser) => {
  ...
  if (testVariant !== 'CONSUMER-LIVE-GMAIL') {
    ...
    ava.default('auto-refresh expired key if newer version of the same key available', testWithBrowser('compose', async (t, browser) => {
      ...
      console.log(await contactsFrame.read('@container-pubkey-details'));
      ...
      await settingsPage.close();
      // above we added an expired key manually
      // now we want to see that it auto-fetches an updated one when trying to use it in compose
      let composePage = await ComposePageRecipe.openStandalone(t, browser, 'compose');
      ...
      // todo - currently does not re-render the expired pubkey even if reloaded in db
      expect(await PageRecipe.getElementPropertyJson(recipient!, 'className')).to.include('expired'); // todo
      // expect(await PageRecipe.getElementPropertyJson(recipient!, 'className')).to.not.include('expired'); // because auto-reloaded
      await composePage.close();
      // now close compose page and try to open it again - should show as green, not orange, because pubkey must have been reloaded earlier 
      composePage = await ComposePageRecipe.openStandalone(t, browser, 'compose');
      await ComposePageRecipe.fillMsg(composePage, { to: recipientEmail }, t.title);
      expandContainer = await composePage.waitAny('@action-show-container-cc-bcc-buttons');
      recipient = await expandContainer.$('.email_preview span');
      expect(await PageRecipe.getElementPropertyJson(recipient!, 'className')).to.not.include('expired'); // because auto-reloaded earlier
    }));
