...
export const defineFlakyTests = (testVariant: TestVariant, testWithBrowser: TestWithBrowser) => {
  ...
  if (testVariant !== 'CONSUMER-LIVE-GMAIL') {
    ...
    ava.default('compose[global:flowcrypt.test.key.new.manual@gmail.com] - standalone - own key expired', testWithBrowser(undefined, async (t, browser) => {
    ...
    ava.default('[standalone] setup - create key advanced - no backup', testWithBrowser(undefined, async (t, browser) => {
      const settingsPage = await BrowserRecipe.openSettingsLoginApprove(t, browser, 'flowcrypt.test.key.new.manual@gmail.com');
