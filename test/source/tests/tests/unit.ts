...
import { PgpMsgUtil, PgpMsgMethod } from '../../core/crypto/pgp/pgp-msg-util';
...
-----END PGP PUBLIC KEY BLOCK-----`;
    ...
    ava.default('[unit][PgpMsgUtil.getSortedKeys,matchingKeyids] must be able to find matching keys', async t => {
      ...
      const { data } = await PgpMsgUtil.encryptMessage({ pubkeys: [pub1], data: Buf.fromUtfStr('anything'), armor: true }) as PgpMsgMethod.EncryptPgpArmorResult;
      ...
      const sortedKeys = await PgpMsgUtil.getSortedKeys(kisWithPp, m);
      ...
      // also test PgpMsgUtil.matchingKeyids
      // @ts-ignore
      const matching1 = await PgpMsgUtil.matchingKeyids(pub1, m.getEncryptionKeyIds());
      expect(matching1.length).to.equal(1);
      // @ts-ignore
      const matching2 = await PgpMsgUtil.matchingKeyids(pub2, m.getEncryptionKeyIds());
