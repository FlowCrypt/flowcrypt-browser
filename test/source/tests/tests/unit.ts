...
import { PgpUtil, PgpMsgMethod } from '../../core/crypto/pgp/pgp-msg';
...
-----END PGP PUBLIC KEY BLOCK-----`;
    ...
    ava.default('[unit][PgpUtil.getSortedKeys,matchingKeyids] must be able to find matching keys', async t => {
      ...
      const { data } = await PgpUtil.encryptMessage({ pubkeys: [pub1], data: Buf.fromUtfStr('anything'), armor: true }) as PgpMsgMethod.EncryptPgpArmorResult;
      ...
      const sortedKeys = await PgpUtil.getSortedKeys(kisWithPp, m);
      ...
      // also test PgpUtil.matchingKeyids
      // @ts-ignore
      const matching1 = await PgpUtil.matchingKeyids(pub1, m.getEncryptionKeyIds());
      expect(matching1.length).to.equal(1);
      // @ts-ignore
      const matching2 = await PgpUtil.matchingKeyids(pub2, m.getEncryptionKeyIds());
