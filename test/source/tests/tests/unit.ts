...
-----END PGP PUBLIC KEY BLOCK-----`;
    ...
    ava.default('[unit][KeyUtil.parse] key was never usable', async t => {
      ...
      expect(parsed?.usableForEncryption).to.be.false;
      expect(parsed?.expiration?.getTime()).to.equal(1594890073000);
      expect(parsed?.usableButExpired).to.be.false; // because last signature was created as already expired, no intersection
      t.pass();
    });
