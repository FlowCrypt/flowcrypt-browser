...
(async () => {
  let result = '';
  try {
    if (!$('[data-test="content-script-test-status"]').length) {
      Xss.sanitizePrepend('body', `<div data-test="content-script-test-status">First run...</div>`);
    }
    Xss.sanitizeAppend('[data-test="content-script-test-status"]', '<div>Running content script tests...</div>');
