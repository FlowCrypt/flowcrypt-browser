...
export class SettingsPageRecipe extends PageRecipe {
  ...
  public static toggleScreen = async (settingsPage: ControllablePage, to: 'basic' | 'additional') => {
    ...
    await settingsPage.waitAndClick(
      to === 'basic' ? '@action-toggle-screen-basic' : '@action-toggle-screen-additional'
    ); // switch
    await Util.sleep(0.5);
  ...
  public static awaitNewPageFrame = async (
    settingsPage: ControllablePage,
    actionBtnSel: string,
    frameUrlFilter: string[]
  ...
  public static changePassphrase = async (
    settingsPage: ControllablePage,
    currentPp: string | undefined,
    newPp: string
  ) => {
    await SettingsPageRecipe.ready(settingsPage);
    ...
    await securityFrame.waitAndRespondToModal(
      'info',
      'confirm',
      'Now that you changed your pass phrase, you should back up your key'
    );
  ...
  public static verifyMyKeyPage = async (
    settingsPage: ControllablePage,
  ...
  public static addKeyTest = async (
    ...
    return await SettingsPageRecipe.addKeyTestEx(
      t,
      browser,
      acctEmail,
      { armoredPrvKey },
  ...
  public static addKeyTestEx = async (
    ...
    const addPrvPage = await browser.newPage(
      t,
      `/chrome/settings/modules/add_key.htm?acctEmail=${Xss.escape(acctEmail)}&parent_tab_id=0`
    );
    ...
    if (checks.isSavePassphraseHidden !== undefined) {
      expect(await addPrvPage.hasClass('@input-save-passphrase-label', 'hidden')).to.equal(
