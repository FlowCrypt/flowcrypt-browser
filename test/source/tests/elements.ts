...
export const defineElementTests = (testVariant: TestVariant, testWithBrowser: TestWithBrowser) => {
  if (testVariant !== 'CONSUMER-LIVE-GMAIL') {
    test(
      'elements/pgp_pubkey.htm renders a public key',
    ...
    test(
      'elements/passphrase.htm shows a toast when unlocking some other key',
      testWithBrowser(async (t, browser) => {
        ...
        const ppPage = await browser.newPage(
          t,
          `chrome/elements/passphrase.htm?acctEmail=${acctEmail}&parentTabId=1%3A0&type=message&longids=ADAC279C95093207`
        );
    ...
    test.todo('elements/pgp_pubkey shows graceful error when pubkey not usable');

    test.todo('elements/pgp_pubkey can render several pubkeys in one armor');

    test.todo('compose - elements/pgp_pubkey can import several pubkeys in one armor');
  }
};
