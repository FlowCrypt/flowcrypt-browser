  ...
  if (testVariant !== 'CONSUMER-LIVE-GMAIL') {
    ...
    for (const inputMethod of ['mouse', 'keyboard']) {
      ...
      ava.default(`compose - pass phrase dialog - dialog cancel (${inputMethod})`, testWithBrowser('ci.tests.gmail', async (t, browser) => {
        ...
        const inboxPage = await browser.newPage(t, TestUrls.extensionInbox(acctEmail));
        const composeFrame = await InboxPageRecipe.openAndGetComposeFrame(inboxPage);
        await ComposePageRecipe.fillMsg(composeFrame, { to: 'anyone@recipient.com' }, 'send signed-only message', undefined, { encrypt: false });
        await composeFrame.waitAndClick('@action-send', { delay: 2 });
        const passphraseDialog = await inboxPage.getFrame(['passphrase.htm']);
    ...
    ava.default('send signed and encrypted S/MIME message', testWithBrowser('compatibility', async (t, browser) => {
      ...
      const inboxPage = await browser.newPage(t, TestUrls.extensionInbox(acctEmail));
      const composeFrame = await InboxPageRecipe.openAndGetComposeFrame(inboxPage);
      await ComposePageRecipe.fillMsg(composeFrame, { to: 'smime@recipient.com' }, 'send signed and encrypted S/MIME message',
    ...
    ava.default('send signed and encrypted S/MIME message entering a non-primary passphrase', testWithBrowser('compatibility', async (t, browser) => {
      const acctEmail = 'flowcrypt.compatibility@gmail.com';
      const passphrase = 'pa$$w0rd';
      await SettingsPageRecipe.addKeyTest(t, browser, acctEmail, testConstants.testKeyMultipleSmimeCEA2D53BB9D24871, passphrase, {}, false);
      const inboxPage = await browser.newPage(t, TestUrls.extensionInbox(acctEmail));
    ...
    ava.default('user4@standardsubdomainfes.test:8001 - PWD encrypted message with FES web portal - some sends fail with BadRequest error', testWithBrowser(undefined, async (t, browser) => {
      ...
      await dbPage.close();
      const subject = 'PWD encrypted message with FES web portal - some sends fail with BadRequest error';
      ...
      await composePage.close();
      expect((await GoogleData.withInitializedData(acct)).searchMessagesBySubject(subject).length).to.equal(1); // + 0 messages
      ...
      await composePage.close();
      expect((await GoogleData.withInitializedData(acct)).searchMessagesBySubject(subject).length).to.equal(2); // + 1 message
      ...
      await composePage.close();
      expect((await GoogleData.withInitializedData(acct)).searchMessagesBySubject(subject).length).to.equal(2); // + 0 messages
      ...
      await composePage.close();
      expect((await GoogleData.withInitializedData(acct)).searchMessagesBySubject(subject).length).to.equal(3); // + 1 message
      ...
      await composePage.close();
      expect((await GoogleData.withInitializedData(acct)).searchMessagesBySubject(subject).length).to.equal(3); // + 0 messages
