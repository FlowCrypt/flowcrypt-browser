    ...
    ava.default(`[unit][MsgBlockParser.detectBlocks] ignores false-positive blocks`, async t => {
      const input = `Hello, sending you the promised json:
      {
        "entries" : [ {
          ...
          "id" : "1,email-key-manager,evaluation.org,pgp-key-public,ekm%40ekm-client-configurations-test.flowcrypt.test",
          ...
          "content" : "1,email-key-manager,evaluation.org,pgp-key-private,106988520142055188323\n1,email-key-manager,evaluation.org,pgp-key-public,ekm%40ekm-client-configurations-test.flowcrypt.test"
        } ]
      }`;
...
=kFcl
-----END PGP PRIVATE KEY BLOCK-----`);
      await t.throwsAsync(() => KeyUtil.decrypt(corruptedRsaKey, '123'), { instanceOf: Error, message: 'Key is invalid' });
      t.pass();
    });
