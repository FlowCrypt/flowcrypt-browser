...
window.flowcrypt_storage.db_open(db => {
    ...
    tool.browser.message.listen_background({
        ...
        db: (request, sender, respond) => db ? db_operation(request, sender, respond, db) : null,
...
window.flowcrypt_storage.get(null, ['errors'], (s) => {
...
function get_active_tab_info(message, sender, respond) {
...
function open_settings_page(path = 'index.htm', account_email = null, page = '', page_url_params = null) {
    let base_path = chrome.extension.getURL(`chrome/settings/${path}`);
    get_cryptup_settings_tab_id_if_open(function (opened_tab) {
        let open_tab = opened_tab ? (url) => { chrome.tabs.update(opened_tab, { url: url, active: true }); } : (url) => { chrome.tabs.create({ url: url }); };
        if (account_email) {
...
function close_popup_handler(q, sender, respond) {
...
function execute_in_background_process_and_respond_when_done(message, sender, respond) {
    ...
    Promise.all(args).then(resolved_args => {
        ...
        if (!has_callback) {
            if (typeof returned === 'object' && typeof returned.then === 'function') {
