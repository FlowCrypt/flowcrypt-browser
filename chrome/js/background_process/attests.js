...
function attest_requested_handler(request, sender, respond) {
    respond();
    refresh_attest_requests_and_privileges(null, () => {
        watch_for_attest_email(request.account_email);
    });
}
function attest_packet_received_handler(request, sender, respond) {
    process_attest_packet_text(request.account_email, request.packet, request.passphrase, function (account, packet, success, result) {
...
function check_email_for_attests_and_respond(account_email) {
    window.flowcrypt_storage.get(account_email, ['attests_requested'], (S) => {
...
function refresh_attest_requests_and_privileges(process_account_email_callback, refresh_done_callback = null) {
...
function is_already_attested(account_email, attester, callback) {
    window.flowcrypt_storage.get(account_email, ['attests_processed'], (S) => {
...
function account_storage_mark_as_attested(account_email, attester, callback) {
    stop_watching(account_email);
    window.flowcrypt_storage.get(account_email, ['attests_requested', 'attests_processed'], (S) => {
        if (tool.value(attester).in(S.attests_requested)) {
            ...
            window.flowcrypt_storage.set(account_email, S, callback);
        }
        else {
            callback();
        }
...
function add_attest_log(account_email, packet, success, attestation_result_text = '', callback = null) {
    ...
    window.flowcrypt_storage.get(account_email, ['attest_log'], (storage) => {
        ...
        window.flowcrypt_storage.set(account_email, storage);
    });
}
