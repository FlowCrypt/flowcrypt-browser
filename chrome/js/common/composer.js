...
(function () {
    ...
    let app = {
        can_read_email: () => true,
        does_recipient_have_my_pubkey: (email, cb) => { if (cb) {
            cb();
        } },
        ...
        storage_get_subscription_info: (cb) => { if (typeof cb === 'function') {
            cb({});
        ...
        storage_contact_get: (email, cb) => { if (cb)
            cb(null); },
        storage_contact_update: (email, update, cb) => { if (cb)
            cb(); },
        storage_contact_save: (contact, cb) => { if (cb)
            cb(); },
        storage_contact_search: (query, cb) => { if (cb)
            cb(); },
        ...
        email_provider_determine_reply_message_header_variables: (cb) => { if (cb)
            cb(); },
        email_provider_extract_armored_block: (message_id, success_cb, error_cb) => { if (error_cb)
            error_cb('not implemented'); },
    ...
    function init(app_functions, variables) {
        ...
        else if (app.storage_get_email_footer()) {
    ...
    function draft_delete(callback) {
        ...
        tool.time.wait(() => { if (!save_draft_in_process) {
            return true;
        } }).then(() => {
            if (draft_id) {
        ...
        add_reply_token_to_message_body_if_needed(recipients, subject, plaintext, challenge, subscription, function (plaintext) {
            handle_send_btn_processing_error(function () {
                attach.collect_and_encrypt_attachments(armored_pubkeys, challenge, function (attachments) {
                    if (attachments.length && challenge) {
                    ...
                    if (passphrase === null) {
                        ...
                        require(['emailjs-mime-codec'], function (MimeCodec) {
                            ...
                            tool.crypto.message.sign(prv, format_email_text_footer({ 'text/plain': plaintext })['text/plain'], true, function (success, signing_result) {
                                if (success) {
                                    handle_send_btn_processing_error(function () {
                                        attach.collect_attachments(function (attachments) {
    ...
    function resize_reply_box(add_extra = null) {
        if (is_reply_box) {
            ...
            if (current_height !== last_reply_box_table_height && Math.abs(current_height - (last_reply_box_table_height || 0)) > 2) {
    ...
    function render_search_results(contacts, query) {
        ...
        renderable_contacts.sort(function (a, b) {
    ...
    function update_pubkey_icon(include = null) {
        if (include === null) {
            if (!include_pubkey_toggled_manually) {
