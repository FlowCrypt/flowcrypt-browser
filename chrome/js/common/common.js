...
let catcher = window.catcher; // to make TS happy
(function () {
    let tool = window['tool'] = {
        ...
        mime: {
            ...
            format_content_to_display: mime_format_content_to_display,
            decode: mime_decode,
            encode: mime_encode,
        ...
        api: {
            ...
            gmail: {
                ...
                draft_send: api_gmail_draft_send,
    ...
    let storage = window.flowcrypt_storage;
    ...
    function str_number_format(nStr) {
        nStr += '';
        let x = nStr.split('.');
        let x1 = x[0];
    ...
    function str_int_to_hex(int_as_string) {
    ...
    function arr_from_dome_node_list(obj) {
        let array = [];
    ...
    function arr_is(object_to_identify) {
    ...
    function file_attachment(name = '', type = 'application/octet-stream', content, size = content.length, url = null) {
        return {
            name: name,
            type: type,
            content: content,
    ...
    function mime_process(mime_message, callback) {
        mime_decode(mime_message, function (success, decoded) {
            // @ts-ignore
            if (typeof decoded.text === 'undefined' && typeof decoded.html !== 'undefined' && typeof $_HOST_html_to_text === 'function') {
                // @ts-ignore
    ...
    function ui_event_prevent(meta, callback) {
        return function () {
            if (meta.name === 'spree') {
    ...
    function ui_spinner(color, placeholder_class = 'small_spinner') {
    ...
    function ui_scroll(selector, repeat) {
        ...
        if (el) {
            el.scrollIntoView();
            tool.each(repeat, function (i, delay) {
                setTimeout(function () {
                    el.scrollIntoView();
                }, delay);
            });
    ...
    function browser_message_send(destination_string, name, data, callback) {
        ...
        if (typeof destination_string === 'undefined') {
                ...
                if (msg.to === listen_for_tab_id || msg.to === 'broadcast') {
                    if (!tool.value(msg.uid).in(processed)) {
                        ...
                        if (typeof frame_registered_handlers[msg.name] !== 'undefined') {
                            ...
                            if (destination_parse(msg.to).frame !== null) {
    ...
    function crypto_armor_strip(pgp_block_text) {
        ...
        if (double_newlines !== null && double_newlines.length > 2) {
            pgp_block_text = pgp_block_text.replace(/\n\n/g, '\n');
            if (debug) {
        ...
        if (begin !== -1) {
            ...
            tool.each(crypto_armor_headers_dict, function (type, block_header) {
                if (block_header.replace) {
                    ...
                    if (index_of_confirmed_begin === 0 || (type === 'password_message' && index_of_confirmed_begin < 15)) {
                        if (begin > start_at) {
    ...
    function crypto_key_decrypt(prv, passphrase) {
        try {
    ...
    function crypto_key_usable(armored) {
    ...
    function crypto_key_fingerprint(key, formatting = 'default') {
    ...
    function get_sorted_keys_for_message(account_email, message, callback) {
        ...
        storage.keys_get(account_email).then(function (private_keys_all) {
            ...
            if (keys.potentially_matching.length === 0) {
    ...
    function zeroed_decrypt_error_counts(keys = null) {
        return {
            decrypted: 0,
    ...
    function crypto_message_verify_detached(account_email, plaintext, signature_text, callback) {
        ...
        if (signature_text instanceof Uint8Array) {
    ...
    function readable_crack_time(total_seconds) {
    ...
    function get_ajax_progress_xhr(progress_callbacks) {
        let progress_reporting_xhr = new window.XMLHttpRequest();
    ...
    function api_common_reply_correspondents(account_email, addresses, last_message_sender, last_message_recipients) {
        ...
        tool.each(reply_to_estimate, function (i, email) {
            if (email) {
                if (tool.value(tool.str.parse_email(email).email).in(addresses)) {
                    my_email = email;
                }
                else if (!tool.value(tool.str.parse_email(email).email).in(reply_to)) {
    ...
    /* tool.api.google */
    let google_oauth2 = typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.getManifest ? chrome.runtime.getManifest().oauth2 : null;
    ...
    function google_auth_window_show_and_respond_to_auth_request(auth_request, current_google_token_scopes, respond) {
        ...
        tool.each(google_oauth2.scopes, function (i, scope) {
            if (!tool.value(scope).in(auth_request.scopes)) {
                if (scope !== tool.api.gmail.scope('read') || !auth_request.omit_read_scope) {
        ...
        function api_google_auth_window_closed_watcher() {
            if (auth_code_window !== null && typeof auth_code_window !== 'undefined' && auth_code_window.closed) {
                clearInterval(window_closed_timer);
    ...
    function google_auth_refresh_token(refresh_token, callback) {
        $.ajax({
            ...
            error: function (XMLHttpRequest, status, error) {
                if (XMLHttpRequest.status === 0 && status === 'error') {
    ...
    function google_auth_window_result_handler(expected_responder_id, result, close_auth_window) {
        if (result.state.auth_responder_id === expected_responder_id) {
            let auth_responder = api_google_auth_responders[result.state.auth_responder_id];
    ...
    function api_google_call(account_email, method, url, parameters, callback, fail_on_auth = false) {
        storage.get(account_email, ['google_token_access', 'google_token_expires'], function (auth) {
            ...
            if (typeof auth.google_token_access !== 'undefined' && auth.google_token_expires > new Date().getTime()) {
                $.ajax({
                    url: url,
                    method: method,
                    data: data,
    ...
    function encode_as_multipart_related(parts) {
    ...
    function api_gmail_message_get(account_email, message_id, format, callback, results = {}) {
    ...
    function file_treat_as(attachment) {
        ...
        else if (attachment.name.match(/(\.pgp$)|(\.gpg$)|(\.[a-zA-Z0-9]{3,4}\.asc$)/g)) {
            return 'encrypted';
        }
        else if (attachment.name.match(/^(0|0x)?[A-F0-9]{8}([A-F0-9]{8})?\.asc$/g)) {
            return 'public_key';
        }
        else if (tool.value('public').in(attachment.name.toLowerCase()) && attachment.name.match(/[A-F0-9]{8}.*\.asc$/g)) {
            return 'public_key';
        }
    ...
    function api_gmail_fetch_attachments(account_email, attachments, callback, results = []) {
    ...
    function api_attester_packet_parse(text) {
        ...
        if (matches && matches[1]) {
            ...
            if (result.error !== null) {
                ...
                if (result.content['fingerprint'] && result.content['fingerprint'].length !== 40) {
    ...
    function api_cryptup_message_upload(encrypted_data_armored, auth_method) {
        return catcher.Promise(function (resolve, reject) {
...
(function () {
    let tool = window['tool'];
    let storage = (typeof exports === 'object') ? require('js/storage').legacy : window.flowcrypt_storage;
    let RUNTIME = {};
    figure_out_flowcrypt_runtime();
    let original_on_error = window.onerror;
    ...
    function handle_promise_error(e) {
        ...
        else if (!tool.value(JSON.stringify(e)).in(['{"isTrusted":false}', '{"isTrusted":true}'])) {
    ...
    function handle_error(error_message, url, line, col, error, is_manually_called, version, env) {
        ...
        if (error_message && url && typeof line !== 'undefined' && !col && !error && !is_manually_called && !version && !env) {
    ...
    function report(name, details = undefined) {
        try {
    ...
    function environment(url = window.location.href) {
    ...
    function cryptup_version(format = 'original') {
        if (format === 'int') {
