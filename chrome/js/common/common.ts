...
(function ( /* ERROR HANDLING */ ) {
  ...
  let tool = window['tool'];
  let storage = (typeof exports === 'object') ? require('js/storage').legacy : (window as FlowCryptWindow).flowcrypt_storage;
  let RUNTIME = {};
  figure_out_flowcrypt_runtime();

  let original_on_error = window.onerror;
  (window as FlowCryptWindow).onerror = (handle_error as any as ErrorEventHandler);
  (window as FlowCryptWindow).onunhandledrejection = handle_promise_error;
  ...
  function handle_error(error_message, url, line, col, error, is_manually_called, version, env) {
    ...
    if(is_manually_called !== true && original_on_error && original_on_error !== (handle_error as any as ErrorEventHandler)) {
  ...
  if((window as FlowCryptWindow).is_bare_engine !== true) {
    (window as FlowCryptWindow).catcher = _c;
    catcher = _c;
  }
...
(function ( /* EXTENSIONS AND CONFIG */ ) {
  ...
  if(typeof (window as FlowCryptWindow).openpgp !== 'undefined' && typeof (window as FlowCryptWindow).openpgp.config !== 'undefined' && typeof (window as FlowCryptWindow).openpgp.config.versionstring !== 'undefined' && typeof (window as FlowCryptWindow).openpgp.config.commentstring !== 'undefined') {
    (window as FlowCryptWindow).openpgp.config.versionstring = 'FlowCrypt ' + (catcher.version() || '') + ' Gmail Encryption flowcrypt.com';
