...
import { Value, Str, Env, Ui, Xss, UnreportableError, Catch } from '../../js/common/common.js';
      ...
      container.find('.action_fix_compatibility').click(Ui.event.handle(async target => {
        ...
        if (!expire_years) {
          ...
          if (await reformatted.key.getEncryptionKey()) {
            ...
            Xss.sanitize_replace(target, Ui.e('a', {href: back_url, text: 'Go back and try something else'}));
          }
        }
      }));
    });
