...
window.flowcrypt_storage.keys_get(url_params.account_email).then(keyinfos => {
      ...
      if(new_key.isPublic()) {
        ...
        if(decrypt_result.error) {
          ...
          Promise.all([
            ...
            window.flowcrypt_storage.passphrase_save($('.input_passphrase_save').prop('checked') ? 'local' : 'session', url_params.account_email, new_key_longid, passphrase),
          ]).then(() => {
