...
tool.catch.try(() => {
  ...
  function collect_info_for_account_backup(account_email: string, callback: (backup_text: string) => void) {
    ...
    (window as FlowCryptWindow).flowcrypt_storage.get(null, ['version'], function (global_storage) {
      (window as FlowCryptWindow).flowcrypt_storage.get(account_email, ['is_newly_created_key', 'setup_date', 'version', 'full_name'], function (account_storage) {
        text.push('global_storage: ' + JSON.stringify(global_storage));
        text.push('account_storage: ' + JSON.stringify(account_storage));
        text.push('');
        (window as FlowCryptWindow).flowcrypt_storage.keys_get(account_email).then(keyinfos => {
  ...
  if(url_params.account_email) {
    (window as FlowCryptWindow).flowcrypt_storage.get(null, ['dev_outlook_allow'], storage => {
    ...
    $('.action_allow_outlook').change(function () {
      (window as FlowCryptWindow).flowcrypt_storage.set(null, {'dev_outlook_allow': $(this).prop('checked')}, storage => {
    ...
    $('.action_fetch_aliases').click(tool.ui.event.prevent(tool.ui.event.parallel(), function(self, id) {
      ...
      fetch_account_aliases_from_gmail(url_params.account_email as string, function(addresses) {
        ...
        (window as FlowCryptWindow).flowcrypt_storage.set(url_params.account_email as string, { addresses: all }, function () {
    ...
    $('.action_flush_attest_info').click(function () {
      (window as FlowCryptWindow).flowcrypt_storage.remove(url_params.account_email as string, ['attests_requested', 'attests_processed', 'attest_log'], function () {
    ...
    $('.action_reset_managing_auth').click(() => {
      (window as FlowCryptWindow).flowcrypt_storage.remove(null, ['cryptup_account_email', 'cryptup_account_subscription', 'cryptup_account_uuid', 'cryptup_account_verified'], () => {
