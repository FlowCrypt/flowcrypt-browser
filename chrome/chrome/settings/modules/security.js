...
window.flowcrypt_storage.keys_get(url_params.account_email, 'primary').then(primary_ki => {
  ...
  $('.confirm_passphrase_requirement_change').click(function () {
    if($('#passphrase_to_open_email').is(':checked')) { // todo - forget pass all phrases, not just master
      window.flowcrypt_storage.passphrase_get(url_params.account_email, primary_ki.longid).then(stored_passphrase => {
        if($('input#passphrase_entry').val() === stored_passphrase) {
          Promise.all([
            window.flowcrypt_storage.passphrase_save('local', url_params.account_email, primary_ki.longid, undefined),
            window.flowcrypt_storage.passphrase_save('session', url_params.account_email, primary_ki.longid, undefined),
      ...
      if(tool.crypto.key.decrypt(key, $('input#passphrase_entry').val()).success) {
        window.flowcrypt_storage.passphrase_save('local', url_params.account_email, primary_ki.longid, $('input#passphrase_entry').val()).then(() => window.location.reload());
      } else {
        alert('Pass phrase did not match, please try again.');
        $('input#passphrase_entry').val('').focus();
      }
