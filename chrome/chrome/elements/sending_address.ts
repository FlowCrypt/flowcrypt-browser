...
tool.catch.try(() => {
  ...
  (window as FlowCryptWindow).flowcrypt_storage.get(url_params.account_email as string, ['addresses'], (storage: {addresses: string[]}) => {
    ...
    container.find('input').click(function() {
      if($(this).val() !== storage.addresses[0]) {
        (window as FlowCryptWindow).flowcrypt_storage.set(url_params.account_email as string, {addresses: tool.arr.unique([$(this).val()].concat(storage.addresses))}, () => window.location.reload());
      }
    });
  });
