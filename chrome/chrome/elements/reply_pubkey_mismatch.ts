...
tool.catch.try(() => {
  ...
  Store.keys_get(url_params.account_email as string, ['primary']).then(([primary_k]) => {
    ...
    tool.each((url_params.to as string).split(','), (i, to) => {
    ...
    tool.api.gmail.thread_get(url_params.account_email as string, url_params.thread_id as string, 'full', (success, thread: any) => {
