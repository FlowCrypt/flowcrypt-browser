...
window.flowcrypt_storage.keys_get(url_params.account_email).then(all_private_keys => {
  ...
  if(url_params.longids) {
    let longids = url_params.longids.split(',');
  ...
  if(all_private_keys.length > 1) {
    ...
    if(selected_private_keys.length === 1) {
      html = 'For the following key: <span class="good">' + mnemonic(selected_private_keys[0].longid) + '</span> (KeyWords)';
    } else {
      html = 'Pass phrase needed for any of the following keys:';
      tool.each(selected_private_keys, function (i, keyinfo) {
