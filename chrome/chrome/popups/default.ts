...
tool.catch.try(() => {
  ...
  tool.browser.message.send(null, 'get_active_tab_info', {}, function (active_tab) {
    if(active_tab && active_tab.account_email !== null) {
      (window as FlowCryptWindow).flowcrypt_storage.get(active_tab.account_email, ['setup_done'], storage => {
      ...
      (window as FlowCryptWindow).flowcrypt_storage.account_emails_get(function (account_emails) {
        if(account_emails && account_emails.length) {
          (window as FlowCryptWindow).flowcrypt_storage.get(account_emails, ['setup_done'], function (account_storages) {
