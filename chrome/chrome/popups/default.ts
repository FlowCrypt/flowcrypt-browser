...
tool.catch.try(async () => {
  ...
  function redirect_to_initial_setup(account_email:string|null=null) {
    tool.browser.message.send(null, 'settings', { account_email }, () => window.close());
  }
  ...
  function set_up_accont_prompt_popup(active_account_email: string) {
    ...
    $('.action_set_up_account').click(tool.ui.event.prevent(tool.ui.event.double(), () => redirect_to_initial_setup(active_account_email)));
  }
  ...
  function choose_email_or_settings_popup(active_account_email:string|null=null) {
    ...
    $('.action_send_email').click(() => {
      if (active_account_email) {
        tool.browser.message.send(null, 'settings', { account_email: active_account_email, page: '/chrome/elements/compose.htm' }, () => window.close());
      } else {
