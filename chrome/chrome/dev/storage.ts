...
tool.catch.try(() => {
  ...
  chrome.storage.local.get(storage => {
    ...
    if(url_params.filter) {
      real_filter = (window as FlowCryptWindow).flowcrypt_storage.key(url_params.filter as string, url_params.keys as string || '') as string;
    } else {
      real_filter = '';
    }
    ...
    $('.save').click(function () {
      try {
        if($('.namespace').val() === '-- namespace --' || $('.type').val() === '-- type --' || !$('.key').val()) {
          ...
          (window as FlowCryptWindow).flowcrypt_storage.set(account_email, storage_update, function () {
            window.location.reload();
          });
        }
      } catch(e) {
