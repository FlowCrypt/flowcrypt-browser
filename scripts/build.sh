...
if [[ "$#" == 1 ]] && [[ "$1" == "--assets-only" ]]; then # only build static assets, without re-building TS
    ( cd $SRCDIR && rsync -R ./**/*.{js,htm,css,woff2,png,svg,txt} ../build/chrome-enterprise )
    ( cd $SRCDIR && rsync -R ./**/*.{js,htm,css,woff2,png,svg,txt} ../build/chrome-consumer )
    ( cd $SRCDIR && rsync -R ./**/*.{js,htm,css,woff2,png,svg,txt} ../build/firefox-consumer )
    exit 0
fi
...
( cd $SRCDIR && rsync -R ./**/*.{js,htm,css,woff2,png,svg,txt} ./{.web-extension-id,manifest.json} ../$OUTDIR )
