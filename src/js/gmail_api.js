...
function gmail_api_call(account, method, resource, parameters, callback, fail_on_auth) {
  account_storage_get(account, 'token', function(token) {
    if(typeof token !== 'undefined') { // have a valid gmail_api oauth token
      $.ajax({
        ...
        error: function(response) {
          ...
          if(typeof error_obj['error'] !== 'undefined' && error_obj['error']['message'] === "Invalid Credentials") {
            gmail_api_handle_auth_error(account, method, resource, parameters, callback, fail_on_auth, response);
          } else {
      ...
      gmail_api_handle_auth_error(account, method, resource, parameters, callback, fail_on_auth, null);
    }
  });
}

function gmail_api_handle_auth_error(account, method, resource, parameters, callback, fail_on_auth, error_response) {
...
function gmail_api_process_postponed_request(signal_data) {
  ...
  gmail_api_call(parameters.account, parameters.method, parameters.resource, parameters.parameters, parameters.callback, true);
}
...
function gmail_api_get_thread(account, thread_id, format, get_thread_callback) {
  gmail_api_call(account, 'GET', 'threads/' + thread_id, {
    format: format
  }, get_thread_callback);
}

function gmail_api_message_send(account, to, subject, thread_id, message, add_headers, message_send_callback) {
  require.config({
    baseUrl: '../../js',
    paths: {
  ...
  require(['emailjs-mime-builder'], function(MimeBuilder) {
    ...
    add_header(headers, 'From', account);
    ...
    gmail_api_call(account, 'POST', 'messages/send', params, message_send_callback);
  });
};
