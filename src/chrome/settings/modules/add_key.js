...
$.each(private_keys, function(i, keyinfo) {
  private_keys_long_ids.push(key_longid(keyinfo.armored));
});
...
$('.action_add_private_key').click(prevent(doubleclick(), function() {
  ...
  if(typeof new_key === 'undefined') {
  ...
  } else if(private_keys_long_ids.indexOf(key_longid(new_key)) !== -1) {
    ...
    if($('#step_2b_manual_enter .input_passphrase_save').prop('checked')) {
      save_passphrase('local', url_params.account_email, key_longid(new_key), $('.input_passphrase').val());
    } else {
